...
static const int8_t g_encoder_dir_cfg[2] = {
    ENCODER_1_DIR_CFG,
    ENCODER_2_DIR_CFG
};

static int8_t Encoder_GetDirectionCfg(uint8_t encoder_num)
{
    switch (encoder_num)
    {
        case ENCODER_1:
            return g_encoder_dir_cfg[0];
        case ENCODER_2:
            return g_encoder_dir_cfg[1];
        default:
            return 1;
    }
}
...
{
    int32_t count = 0;
    int8_t direction_cfg = Encoder_GetDirectionCfg(encoder_num);
