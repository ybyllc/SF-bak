...
/* USER CODE BEGIN 0 */
static const int8_t g_motor_dir_cfg[4] = {
  MOTOR_FRONT_LEFT_DIR_CFG,
  MOTOR_FRONT_RIGHT_DIR_CFG,
  MOTOR_BACK_LEFT_DIR_CFG,
  MOTOR_BACK_RIGHT_DIR_CFG
};

static uint8_t Motor_ApplyDirectionConfig(uint8_t motor, uint8_t direction)
{
  if (motor < 4 && g_motor_dir_cfg[motor] == -1) {
    return (direction == MOTOR_DIR_FORWARD) ? MOTOR_DIR_BACKWARD : MOTOR_DIR_FORWARD;
  }
  return direction;
}
...
void Motor_SetSpeed(uint8_t motor, int16_t speed)
{
  ...
  /* 根据配置反转逻辑方向（用于适配电机接线接反） */
  direction = Motor_ApplyDirectionConfig(motor, direction);
