...
#define MENU_SPEED_TEST_TARGET_CPS_DEFAULT   500.0f
/* 小小车PID
#define MENU_SPEED_PID_KP_DEFAULT            0.45f
#define MENU_SPEED_PID_KI_DEFAULT            0.02f
#define MENU_SPEED_PID_KD_DEFAULT            0.00f
*/
/* R5底盘PID */
#define MENU_SPEED_PID_KP_DEFAULT            1.15f
#define MENU_SPEED_PID_KI_DEFAULT            0.05f
#define MENU_SPEED_PID_KD_DEFAULT            0.00f
#define MENU_SPEED_PID_OUT_MAX               1000.0f
#define MENU_SPEED_PID_OUT_MIN               (-1000.0f)
...
#define MENU_CIRCLE_CTRL_PERIOD_MS           5U
#define MENU_FAST_CTRL_STALL_TIMEOUT_MS      1000U
#define MENU_CIRCLE_TARGET_RADIUS_CM         20.0f
#define MENU_CIRCLE_BASE_PWM                 260
#define MENU_CIRCLE_TARGET_RATE_DPS          14.0f
#define MENU_CIRCLE_PID_KP                   2.0f
#define MENU_CIRCLE_PID_KI                   0.03f
#define MENU_CIRCLE_PID_KD                   0.10f
...
#define MENU_CIRCLE_HEADING_LPF_ALPHA_MIN    0.60f
#define MENU_CIRCLE_HEADING_LPF_ALPHA_MAX    0.92f
#define MENU_CIRCLE_HEADING_LPF_GZ_REF_DPS   80.0f

//相机追踪 运动参数
#define MENU_CAM_TRACK_CENTER_X              (-100)
#define MENU_CAM_TRACK_X_DEADBAND            10
#define MENU_CAM_TRACK_BASE_PWM              320
#define MENU_CAM_TRACK_STEER_KP              0.16f
...
#define MENU_CAM_TRACK_TARGET_STEP_MAX_DEG   3.5f
#define MENU_CAM_TRACK_X_MAX_DEG             60.0f
...
#define MENU_CAM_TRACK_TURN_BRAKE_GAIN       0.70f
#define MENU_CAM_TRACK_BASE_PWM_MIN          220
...
static float Menu_CalcHeadingLpfAlpha(float gyro_z_dps);
...
static void Menu_SpeedStraight_ControlStep(void);
static void Menu_GyroCircle_ControlStep(void);
static void Menu_RunFastControlStep(void);
...
static float Menu_CalcHeadingLpfAlpha(float gyro_z_dps)
{
    ...
    rate_abs = fabsf(gyro_z_dps);
    ratio = rate_abs / MENU_CIRCLE_HEADING_LPF_GZ_REF_DPS;
    ratio = Menu_ClampFloat(ratio, 0.0f, 1.0f);

    return MENU_CIRCLE_HEADING_LPF_ALPHA_MIN +
           (MENU_CIRCLE_HEADING_LPF_ALPHA_MAX - MENU_CIRCLE_HEADING_LPF_ALPHA_MIN) * ratio;
}

void Menu_RequestFastControlTick(void)
{
    /* TIM1_100us_Callback -> 每1ms触发一次本函数，直接执行控制步进 */
    Menu_RunFastControlStep();
}
...
static void Menu_GyroCircle_ControlStep(void)
{
    ...
    if (dt_ms > MENU_FAST_CTRL_STALL_TIMEOUT_MS) {
        PID_Reset(&gyro_circle_heading_pid);
        gyro_circle_turn_output = 0.0f;
        gyro_circle_pwm_left = 0;
        gyro_circle_pwm_right = 0;
    ...
    heading_alpha = Menu_CalcHeadingLpfAlpha(fGyro[2]);
    gyro_circle_filtered_heading = Menu_NormalizeAngle(
        gyro_circle_filtered_heading + heading_delta * heading_alpha);
...
static void Menu_RunFastControlStep(void)
{
    // 定时触发的快速控制路径：独立于页面刷新频率
    Menu_SpeedStraight_ControlStep();
    Menu_GyroCircle_ControlStep();
    Menu_UpdateCamTrack();
}
...
static void Menu_UpdateCamTrack(void) {
    ...
    float heading_alpha;
    int16_t cam_base_pwm_dynamic;
    int16_t turn_brake_pwm;
    ...
    cam_track_filtered_heading = Menu_NormalizeAngle(
        cam_track_filtered_heading + heading_delta * heading_alpha);
    ...
    if ((cam->is_online != 0U) &&
        ...
        // 大转向时自动小幅降速，缩小转弯半径，直线时保持较高速度
        turn_brake_pwm = (int16_t)(fabsf(cam_track_turn_output) * MENU_CAM_TRACK_TURN_BRAKE_GAIN);
        cam_base_pwm_dynamic = (int16_t)(MENU_CAM_TRACK_BASE_PWM - turn_brake_pwm);
        if (cam_base_pwm_dynamic < MENU_CAM_TRACK_BASE_PWM_MIN) {
            cam_base_pwm_dynamic = MENU_CAM_TRACK_BASE_PWM_MIN;
        }

        cam_track_pwm_left = Menu_ClampMotorPwm((int32_t)cam_base_pwm_dynamic - (int32_t)cam_track_turn_output);
        cam_track_pwm_right = Menu_ClampMotorPwm((int32_t)cam_base_pwm_dynamic + (int32_t)cam_track_turn_output);
...
void Menu_Run(void) {
    ...
    if (key_pc6_event != 0) {
        if (key_pc6_event == 1) {
            if (menuState.currentPage == MENU_PAGE_CAM_INFO) {
                // 在Cam Info页面：启停相机追踪任务
                if (cam_track_active == 0U) {
                    OLED_Clear(0);
                    OLED_ShowString_Reverse(24, 3, "Cam Track Start", 12);
                    OLED_Refresh();
                    HAL_Delay(500);
                }
                ...
                if (cam_track_active == 0U) {
                    OLED_Clear(0);
                    OLED_ShowString(32, 3, "Cam Track Stop", 12);
                    OLED_Refresh();
                    HAL_Delay(500);
                    OLED_Clear(0);
                }
...
void Menu_CamTrack_HandleShortPress(void) {
    ...
    PID_Reset(&cam_track_steer_pid);
    PID_Init(&cam_track_heading_pid, MENU_CIRCLE_PID_KP, MENU_CIRCLE_PID_KI, MENU_CIRCLE_PID_KD,
...
static void Menu_DisplayCamInfoPage(void) {
    ...
    OLED_Clear(0);
    snprintf(str, sizeof(str), "OK:%lu %s PC6:%s",
             (unsigned long)cam.frame_ok,
             (cam_track_active != 0U) ? "ON" : "OFF",
             (cam_track_active != 0U) ? "Stop" : "Go");
