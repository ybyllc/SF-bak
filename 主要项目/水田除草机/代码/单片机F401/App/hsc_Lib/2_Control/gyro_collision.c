...
/* ------------  姿态滤波（控制层算法）  ------------ */
#define GYRO_FILTER_GYRO_ALPHA_XY       0.10f
#define GYRO_FILTER_GYRO_ALPHA_Z_MIN    0.28f
#define GYRO_FILTER_GYRO_ALPHA_Z_MAX    0.55f
#define GYRO_FILTER_GYRO_ALPHA_Z_REF    120.0f
#define GYRO_FILTER_ACC_ALPHA           0.10f
#define GYRO_FILTER_COMP_ALPHA          0.98f
#define GYRO_FILTER_BIAS_LERP_STATIC_XY 0.010f
#define GYRO_FILTER_BIAS_LERP_STATIC_Z  0.020f
#define GYRO_FILTER_STILL_GYRO_THR      6.0f
#define GYRO_FILTER_STILL_ACC_THR       0.15f
...
void Gyro_Filter_Update(float *acc_g, float *gyro_dps, float *angle_deg, float *yaw_deg)
{
    ...
    if (!s_gyro_filter.inited) {
        ...
        for (i = 0; i < 3; i++) {
            // 首次进入时直接把当前陀螺仪读数作为初始零偏，减少起步漂移
            s_gyro_filter.gyro_bias[i] = gyro_dps[i];
            s_gyro_filter.gyro_lpf[i] = 0.0f;
    ...
    gyro_abs_max = fmaxf(fabsf(gx), fmaxf(fabsf(gy), fabsf(gz)));

    // 静止时慢速更新零偏，降低积分漂移
    if ((fabsf(acc_norm - 1.0f) < GYRO_FILTER_STILL_ACC_THR) &&
        (gyro_abs_max < GYRO_FILTER_STILL_GYRO_THR)) {
        s_gyro_filter.gyro_bias[0] += GYRO_FILTER_BIAS_LERP_STATIC_XY * (gx - s_gyro_filter.gyro_bias[0]);
        s_gyro_filter.gyro_bias[1] += GYRO_FILTER_BIAS_LERP_STATIC_XY * (gy - s_gyro_filter.gyro_bias[1]);
        s_gyro_filter.gyro_bias[2] += GYRO_FILTER_BIAS_LERP_STATIC_Z * (gz - s_gyro_filter.gyro_bias[2]);
    }
    ...
    gz_alpha_ratio = fabsf(gz) / GYRO_FILTER_GYRO_ALPHA_Z_REF;
    if (gz_alpha_ratio > 1.0f) {
        gz_alpha_ratio = 1.0f;
    }
    gz_alpha = GYRO_FILTER_GYRO_ALPHA_Z_MIN +
               (GYRO_FILTER_GYRO_ALPHA_Z_MAX - GYRO_FILTER_GYRO_ALPHA_Z_MIN) * gz_alpha_ratio;

    s_gyro_filter.gyro_lpf[0] += GYRO_FILTER_GYRO_ALPHA_XY * (gx - s_gyro_filter.gyro_lpf[0]);
    s_gyro_filter.gyro_lpf[1] += GYRO_FILTER_GYRO_ALPHA_XY * (gy - s_gyro_filter.gyro_lpf[1]);
    s_gyro_filter.gyro_lpf[2] += gz_alpha * (gz - s_gyro_filter.gyro_lpf[2]);
