...
/*
 * 串口调参文件
 * 目的是AI能读取串口信息来了解小车运动状态，
 * 陀螺仪状态和电机状态等，能够动态调参PID等。
 * 让AI自动调参PID，接收若干个数据后调节PID参数
*/
...
static void Debug_Uart_SendLine(const char *fmt, ...)
{
    ...
    (void)HAL_UART_Transmit(s_debug_huart, (uint8_t *)s_tx_buf, (uint16_t)len, DEBUG_UART_TX_TIMEOUT_MS);
}
